...
function buildJsonFromWorkbook(workbook) {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    rows.forEach((row) => {
      ...
      if (!output[countrySlug][category]) {
        output[countrySlug][category] = [];
      }

      output[countrySlug][category].push({
        name,
        type: String(
          getField(row, ["Type", "Short Type"]) || ""
        ).trim(),
        ...
        web: String(
          getField(row, ["Website", "Web"]) || ""
        ).trim(),

        android: String(
          getField(row, ["Android", "Android Link"]) || ""
        ).trim(),

        ios: String(
          getField(row, ["iOS", "IOS", "Apple", "Apple Link"]) || ""
        ).trim(),

        badges: [
          String(
            getField(row, ["Badge1", "Badge 1"]) || ""
          ).trim(),

          String(
            getField(row, ["Badge2", "Badge 2"]) || ""
          ).trim(),

          String(
            getField(row, ["Badge3", "Badge 3"]) || ""
          ).trim()
        ].filter(Boolean),

        docs: [
          String(
            getField(row, ["Instruction1", "Instruction 1"]) || ""
          ).trim(),

          String(
            getField(row, ["Instruction2", "Instruction 2"]) || ""
          ).trim(),

          String(
            getField(row, ["Instruction3", "Instruction 3"]) || ""
          ).trim()
        ].filter(Boolean)
      });
    });
...
export default function AdminPage() {
  ...
  const [status, setStatus] = useState(
    "Upload your Excel file to preview and export JSON."
  );
  ...
  async function checkSession() {
    try {
      ...
      setAuthState("logged-out");
      setAuthStatus(
        "Log in to manage country data, blogs, FAQs and shop products."
      );
    } catch {
      setAuthState("logged-out");
      setAuthStatus(
  ...
  async function login(event) {
    ...
    try {
      ...
      setAuthStatus(
        "Logged in. Country data, blogs, FAQs and shop products are now protected."
      );
    } catch (error) {
  ...
  async function handleFile(event) {
    ...
    try {
      ...
      const converted =
        buildJsonFromWorkbook(workbook);
      ...
      setStatus(
        "Excel converted successfully. You can now export JSON."
      );
    } catch (error) {
      console.error(error);

      setStatus(
        "Error reading Excel file. Please check the format and try again."
      );
    }
  }
  ...
  function downloadJson() {
    ...
    setStatus(
      "JSON downloaded. You can use it to update your website data."
    );
  }
      ...
      <main className="min-h-screen bg-white px-6 py-12">
        <div className="mx-auto max-w-3xl rounded-[2rem] border border-amber-200 bg-amber-50 p-8 text-amber-900 shadow-soft">
          ...
          <p className="mt-3">
            Add ADMIN_USERNAME,
            ADMIN_PASSWORD and
            ADMIN_SESSION_SECRET in
            Vercel Environment
            Variables, then redeploy.
          </p>

          <p className="mt-5 rounded-2xl bg-white/70 p-4">
            {authStatus}
          </p>
      ...
      <main className="min-h-screen bg-white px-6 py-12">
        ...
        <div className="mx-auto max-w-3xl rounded-[2rem] border border-gray-200 bg-white p-8 shadow-soft">
          ...
          <h1 className="text-4xl font-black tracking-[-0.05em]">
            Aliwvide Admin Login
          </h1>

          <p className="mt-3 text-gray-600">
            Log in to manage country data,
            blogs, FAQs and shop products.
          </p>
          ...
          >
            ...
            <button
              ...
              className="rounded-2xl bg-gray-950 px-5 py-3 font-bold text-white disabled:opacity-60"
            >
              {authLoading
                ? "Logging in..."
                : "Login"}
          ...
          <p className="mt-5 rounded-2xl bg-gray-50 p-4 text-gray-700">
            {authStatus}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div>
            ...
            <h1 className="text-5xl font-black tracking-[-0.07em] md:text-6xl">
              Aliwvide Secure Data Manager
            </h1>

            <p className="mt-5 max-w-3xl text-lg leading-8 text-gray-600">
              Manage country app data,
              publish live blogs,
              update FAQs and manage shop
              affiliate links from one
        ...
        {activeSection ===
          ...
          <>
            <div className="mt-8 rounded-[2rem] border border-gray-200 bg-white p-8 shadow-soft">
              ...
              <p className="mt-4 rounded-2xl bg-gray-50 p-4 text-gray-700">
                {status}
              </p>
                          ...
                          {Object.keys(
                            ...
                            (
                              ...
                              <td
                                ...
                                className="max-w-[280px] whitespace-nowrap px-4 py-3 text-gray-600"
                              >
                                {String(
                                  row[
                                    key
            ...
            <div className="mt-8 rounded-[2rem] bg-blue-50 p-8 text-blue-950">
              ...
              <p className="mt-3">
                Country app data still
                exports as JSON. Blogs,
                FAQs and shop affiliate
                links support live
                Supabase management
                from their admin tabs.
              </p>
