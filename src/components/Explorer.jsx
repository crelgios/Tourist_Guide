...
import { ExternalLink, Globe2, Smartphone, Apple } from "lucide-react";
...
export default function Explorer() {
  ...
  if (step === "category") {
    return (
      <StepShell>
        <div className="glass w-full max-w-4xl rounded-[2rem] p-8 shadow-soft">
            ...
            {categories.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <strong className="mt-3 block">{labels[0]}</strong>
                  <span className={category === item.key ? "text-gray-300" : "text-gray-500"}>{labels[1]}</span>
                </button>
              );
            })}
          </div>
          <button onClick={() => { setSelectedIndex(0); setStep("results"); }} className="mt-8 w-full rounded-full bg-gray-950 px-10 py-4 font-bold text-white">{t.showRecommended}</button>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[360px_1fr]">
        <aside className="overflow-hidden rounded-[2rem] border border-gray-200 bg-white shadow-soft">
          <div className="border-b border-gray-200 p-6">
            ...
            <p className="text-gray-500">{countryName} • {categoryLabel[0]} • {language}</p>
          </div>
